...
describe('express-rest-mongo {envelope:true}', function () {
    ...
    app = createApp('mongodb://localhost:27017/express-rest-mongo-test2', {envelope:true})
    db = app.db
    db.bind('users')
/*
    after(function (done) {
    ...
    describe('/:collection/:id', function () {
        ...
        describe('PUT', function () {
            it('should update document', function (done) {
                request(app).put('/api/v1/users/0004')
                    .set('Content-Type', 'application/json')
                    .send({name:'Mary', email:'mary@example.com'})
                    .expect(200)
                    .end(function(err, res) {
                        if (err) return done(err)
                        var result = JSON.parse(res.text)
                        assert.ok(result.user, 'expect envelope')
                        assert.equal(result.user.id, '0004')
                        assert.notOk(result.user._id);
                        db.users.findOne({_id: '0004'}, function (e, result) {
                            assert.equal(result.name, 'Mary');
                            done()
                        })
                    })
            })
            ...
            it('can return no envelope', function (done) {
                request(app).put('/api/v1/users/0004?envelope=false')
                    .set('Content-Type', 'application/json')
